...
export default class YouTubePlayer extends EventEmitter {
	constructor(element, opts) {
		...
		log.info('Initializing YouTube Player...');

		const elem = typeof element === 'string' ? document.querySelector(element) : element;

		log.info('Initializing YouTube Player:', elem);
	...
	load(videoId, autoplay = false, start = 0) {
		...
		log.info('YTPlayer:load', videoId, autoplay, start);
	...
	_onError(data) {
		...
		if (code === YOUTUBE_ERROR.UNPLAYABLE_1 || code === YOUTUBE_ERROR.UNPLAYABLE_2 || code === YOUTUBE_ERROR.NOT_FOUND || code === YOUTUBE_ERROR.INVALID_PARAM) {
			return this.emit('unplayable', { videoId: this.videoId, errorCode: code, data: data });
		}
